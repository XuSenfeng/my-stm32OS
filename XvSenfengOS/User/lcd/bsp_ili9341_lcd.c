...
void putblock8_8(uint16_t x0, uint16_t y0, int pxsize,int pysize, uint16_t *buf)
{
	...
	uint16_t color;
	//在鼠标之前的位置进行绘制
	if(Mouse_def.x_old==Mouse_def.x && Mouse_def.y_old == Mouse_def.y)
		return;
	//计算鼠标在桌面内的长和宽
	if((Mouse_def.x_old+Mouse_def.Width)>ILI9341_MORE_PIXEL)
		width =  ILI9341_MORE_PIXEL - Mouse_def.x_old;
	if((Mouse_def.y_old+Mouse_def.High)>ILI9341_LESS_PIXEL)
		high = ILI9341_LESS_PIXEL - Mouse_def.y_old;
	
	ILI9341_OpenWindow ( Mouse_def.x_old, Mouse_def.y_old, width, high );
	ILI9341_Write_Cmd ( CMD_SetPixel );
	for (y = 0; y < high; y++)
	{
		//获取保存的桌面的数据
	...
	//获取之前的颜色数据
	if((Mouse_def.x+Mouse_def.Width)>ILI9341_MORE_PIXEL)
		width =  ILI9341_MORE_PIXEL - Mouse_def.x;
	if((Mouse_def.y+Mouse_def.High)>ILI9341_LESS_PIXEL)
		high = ILI9341_LESS_PIXEL - Mouse_def.y;
	printf("width = %d, high = %d, x = %d, y = %d\n", width, high, Mouse_def.x,Mouse_def.y);
	//绘制新的鼠标
	ILI9341_OpenWindow ( Mouse_def.x, Mouse_def.y, width, high );
	ILI9341_Write_Cmd ( CMD_SetPixel );	
